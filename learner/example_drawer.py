	...
	def paintEvent(self, event):
		...
		for i in range(len(self.demo)):
			...
			if self.buttons_allowed:
				...
				qp.setFont(QFont('Source Sans Pro', self.font_size-3, QFont.Light, italic=True))
				rect = QRectF(start_x + (i+1)*move_x + 10,self.height()/2 - self.rect_height/2 + int(round(9.0*self.rect_height/16)),self.rect_width - 20,20)
			...
			# label the input
			rect = QRectF(start_x + (i)*move_x + rect_width, self.height()/2 - 12.0*self.rect_height/40, move_x-rect_width, 22)
			qp.setPen(QColor(0, 0, 0))
			qp.setFont(QFont('Source Sans Pro', self.font_size, QFont.Light, italic=True))
