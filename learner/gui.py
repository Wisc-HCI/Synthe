...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.bodystorm_history = BodystormHistory("Bodysessions", parent=self.plotlabel1, frame=self)
        ...
        elif sys.argv[3] == "cmd_input":
            self.button.clicked.connect(self.command_line_record)
        else:
            print("GUI >> ERROR: unknown argument, idx 2")
            exit(1)
        ...
        connectButton = QAction('Connect Mics', self)
        connectButton.triggered.connect(self.connector.on_click)
        closeMicButton = QAction('Disconnect Mics', self)
        closeMicButton.triggered.connect(self.connector.close_socket)
        connectRobotButton = QAction('Connect Robot', self)
        connectRobotButton.triggered.connect(self.robot_connector.on_click)
        connectArmbandButton = QAction('Connect Armbands', self)
        ...
        fileMenu.addAction(closeMicButton)
        fileMenu.addAction(connectRobotButton)
        fileMenu.addAction(connectArmbandButton)
    ...
    def resizeWindow(self):
        if self.initialized:
            ...
            #print("resizing -- {} {}".format(self.width, self.height))
    ...
    def setup_new_design(self):
        self.slider.sp.setValue(5)

        # decide on the design
        speech_filename = sys.argv[2]
        design = ""
        if "delivery" in speech_filename:
            design = "Delivery"
        elif "desk" in speech_filename:
            design = "Information_Desk"
        elif "ticket" in speech_filename:
            design = "Ticket_Booth"
        else:
            print("GUI >> ERROR: design undefined")
            exit()
        self.bodystorm = Bodystorm(design)
        self.solution = None
    ...
    def record(self):
        ...
        if self.button.isChecked():
            ...
            status = self.bodystorm_listener.start_record()
            ...
            _thread.start_new_thread(self.bodystorm_listener.end_record, ("end_thread_speech",))
    ...
    def update_solution(self, solution,n):
        ...
        if solution is None:
            ...
            print("PERFORMANCE TESTER: adding stuff")
    ...
    def update_representation(self, text):
        ...
        if text == "Movable":
            ...
            self.font_selector.setValue(12)
            self.webView.load_graph("basic")
            self.webView.show()
        elif text == "Clean":
            self.traceView.hide()
        ...
        '''
            ...
            self.webView.hide()
            self.traceView.hide()
            self.dotview.load_image(self.label, "fig/RecordNotification.png")
        elif text == "blank":
            self.webView.load_blank_html()
            self.webView.show()
        elif text == "wait":
            self.webView.hide()
            self.traceView.hide()
            self.dotview.load_image(self.label, "fig/wait.png")
        '''
    ...
    def update_names(self, p1, p2):
        self.participant1 = p1
        self.participant2 = p2

        #print(self.participant1)
        #print(self.participant2)
    ...
    def performance_tester(self):
        solver = SMTSolver(self.progress_bar, None, -1)
        inps = [2,3,4,5,6,7,8,9]
        inp_num = [1,2,3,4,5,6,7,8]
        for i in range(0,len(inps)):   # length of inputs
            arr = np.zeros((8,10))
            for j in range(0,len(inp_num)): # number of of the inputs
                for k in range(0,10):  # number of samples

                    with open("perf.txt", "a") as outfile:
                        outfile.write("ITERATION: inp len = {}, inp num = {}, k = {}\n".format(inps[i],inp_num[j],k))

                    # calculate a demo
                    for l in range(inp_num[j]):
                        demo_list = []
                    ...
                    '''
                    print("UPDATING SOLUTION: {}".format(self.solution))
                    self.update_model(self.slider.value(), True, expand=False)
                    print("UPDATED SOLUTION: {}".format(self.solution))
                    ...
                    '''
                    solver.solve(self.bodystorm, 5)
                    self.solution = solver.solution
                    arr[j][k] = self.solution.time
            ...
            with open("performance{}.pkl".format(inps[i]), "wb") as fp:
                pickle.dump(arr, fp)
