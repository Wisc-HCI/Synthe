...
class GestureRecognizer():
	...
	def __init__(self, arg=False):
		...
		self.debug = arg
		self.return_data = "start: {} data: {}".format("-1", '')

	def end_recording(self):
		print("ARMBAND >> end recording received")
		self.recording = False

	def begin_record(self):
		self.recording = True
		self.gthread = Thread(target=self.gesture_thread)
		self.gthread.start()
	...
	def gesture_thread(self):
		...
		if len(ts_data) == 0 and self.debug != "debug":
			...
			if self.debug != "debug":
				...
				self.return_data = "start: {} data: {}".format(start_time, chunks)
				print("GESTURES --> done sending data")

		print("ARMBAND >> ended analysis")
		self.done_recognizing.emit()
